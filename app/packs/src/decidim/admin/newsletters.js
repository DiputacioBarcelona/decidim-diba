...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectors = {
    ...
    sendToSelectedUsers: document.querySelector("#newsletter_send_to_selected_users"),
    ...
    copySelectedUsers: document.querySelector(".newsletter_copy-selected-users")
  };
  ...
  const attachEventListeners = () => {
    ...
    selectors.copySelectedUsers?.addEventListener("change", async (event) => {
      const newsletterId = event?.target?.value;
      const tomselect = document.getElementById("users-ids-select")?.tomselect;
      if (!newsletterId) {
      ...
      try {
        const response = await fetch(`/newsletter_settings?newsletter_id=${encodeURIComponent(newsletterId)}`, {
          method: "GET",
        ...
        const selectedUserIds = Array.isArray(data?.message) ? data.message : [];
        tomselect.setValue(selectedUserIds, true);
        tomselect.refreshItems();
      } catch (error) {
